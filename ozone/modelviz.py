...
body_template = """
  ...
  {% for model in models %}
    ...
    {% if not disable_fields %}
        ...
        ><FONT {% if field.high %}COLOR="#7B7B7B" {% endif %}FACE="Helvetica Bold">{{ field.name }}</FONT
        ></TD>
        <TD ALIGN="LEFT"
        ><FONT {% if field.high %}COLOR="#7B7B7B" {% endif %}FACE="Helvetica Bold">{{ field.type }}</FONT
        ></TD></TR>
        {% endfor %}
    {% endif %}
    </TABLE>
...
def generate_dot(app_labels, **kwargs):
    ...
    for app_label in app_labels:
        ...
        for appmodel in get_models(app):
            model = {
                'name': appmodel._meta.verbose_name or appmodel.__name__,
                'fields': [],
                'relations': []
                }
            ...
            def add_attributes():
                model['fields'].append({
                    ...
                    #'high': not field.editable or field.primary_key,
                    'high': field.blank,
                    })
            ...
            def add_relation(extras=""):
                _rel = {
                    'target': field.rel.to._meta.verbose_name or field.rel.to.__name__,
                    'type': type(field).__name__,
                    'name': field.verbose_name or field.name,
                    'arrows': extras
                    }
...
def main():
    ...
    print generate_dot(args, **kwargs).encode('utf-8')
